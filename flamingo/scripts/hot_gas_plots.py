...
import interpolate_X_Ray_master as xray_new
...
def compute_Xs_mu(elements_array):
    ...
    for i in range(len(all_elements) - 1):
        ne_nH += (
            elements_array[:, i + 1]
            * (element_masses[0] / element_masses[i + 1])
            * (agtomic_num[i + 1] / agtomic_num[0])
        )
        ni_nH += elements_array[:, i + 1] * (
            element_masses[0] / element_masses[i + 1]
        )
...
def read_hot_gas_props(
    run_directory, snapshot_name, cat_name, obs_data_dir, X_Ray_table, chunk
):
    ...
    for index, row in chunk.iterrows():
        ...
        particles, unbound_particles = groups.extract_halo(
            halo_id=int(row.GroupNumber)
        )
        data, bmask = to_swiftsimio_dataset(
            particles,
            run_directory + "/" + snapshot_name,
            generate_extra_mask=True,
        )
        udata, umask = to_swiftsimio_dataset(
            unbound_particles,
            run_directory + "/" + snapshot_name,
            generate_extra_mask=True,
        )
        ...
        for element in elements:
            df_elements[element] = getattr(
                data.gas.smoothed_element_mass_fractions, element
            )

        df_gas = pd.DataFrame(
            data.gas.coordinates.value, columns=["x", "y", "z"]
        )
        df_gas["temp"] = data.gas.temperatures.value
        df_gas["mass"] = data.gas.masses.to("Msun").value.astype(
            np.float64
        ) * u.Msun.to(u.gram)
        ...
        df_gas.x.loc[df_gas.x > BoxSize] = (
            df_gas.x.loc[df_gas.x > BoxSize] - BoxSize
        )
        df_gas.y.loc[df_gas.y > BoxSize] = (
            df_gas.y.loc[df_gas.y > BoxSize] - BoxSize
        )
        df_gas.z.loc[df_gas.z > BoxSize] = (
            df_gas.z.loc[df_gas.z > BoxSize] - BoxSize
        )
        df_gas["distance"] = (
            ((BoxSize / 2) - df_gas.x) ** 2
            + ((BoxSize / 2) - df_gas.y) ** 2
            + ((BoxSize / 2) - df_gas.z) ** 2
        ) ** 0.5
        ...
        df_gas["ypar"] = (
            (sigma_t / (511 * ergs_keV))
            * kb
            * df_gas.temp
            * (df_gas.mass.values * 0.752 * df_gas.ne_nH / mp)
            / mpc
            / mpc
        )  # in Mpc**2
        df_gas["ypar"] = df_gas.ypar * (ez ** (-2.0 / 3))
...
def plot_hot_gas_props(arguments):
    ...
    sun_ne = pd.read_csv(
        obs_data_dir + "Sun2009_ne_profiles.txt",
        skiprows=4,
        delim_whitespace=True,
        names=["log_r", "log_ne", "log_err_m", "log_err_p"],
    )
    sun_P = pd.read_csv(
        obs_data_dir + "Sun2011_P_profiles.txt",
        skiprows=4,
        delim_whitespace=True,
        names=["log_r", "log_P", "log_err_m", "log_err_p"],
    )
    ...
    for run_name, run_directory, snapshot_name, cat_name in zip(
        ...
        try:
            ...
            df_halo[
                "structuretype"
            ] = catalogue.structure_type.structuretype  # Type of structure
            df_halo[
                "m500"
            ] = catalogue.spherical_overdensities.mass_500_rhocrit.to(
                "Msun"
            )  # M500 in Msun
            df_halo["r500"] = (
                catalogue.spherical_overdensities.r_500_rhocrit.to("Mpc") / aexp
            )  # co-moving r500 in Mpc
            df_halo["m500"] = df_halo.m500 * hse_bias
            df_halo["r500"] = df_halo.r500 * hse_bias ** (1.0 / 3)
            df_halo["CoP_x"] = (
                catalogue.positions.xcminpot.to("Mpc") / aexp
            )  # co-moving coordinates in Mpc
            df_halo["CoP_y"] = (
                catalogue.positions.ycminpot.to("Mpc") / aexp
            )  # co-moving coordinates in Mpc
            df_halo["CoP_z"] = (
                catalogue.positions.zcminpot.to("Mpc") / aexp
            )  # co-moving coordinates in Mpc
            df_halo["kT500"] = (
                G
                * df_halo.m500
                ...
                * mu
                * mp
                / (2 * df_halo.r500 * aexp * u.Mpc.to(u.cm))
                / ergs_keV
            )  # keV
            df_halo["rho500"] = 500 * rho_crit * fb  # g/cm^3
            df_halo["Pe500"] = (
                df_halo.kT500 * ergs_keV * ne_n * (df_halo.rho500 / (mu * mp))
            )  # ergs/cm^3
            df_halo["ne500"] = df_halo.Pe500 / (
                df_halo.kT500 * ergs_keV
            )  # cm^-3
            df_halo["S500"] = df_halo.kT500 / (df_halo.ne500) ** (
                2.0 / 3.0
            )  # keV cm^2
            ...
            chunks = [
                df_halo.iloc[df_halo.index[j : j + chunk_size]]
                for j in range(0, n_halos, chunk_size)
            ]
            ...
            medians, bin_edges, binnumber = stats.binned_statistic(
                np.log10(halos.m500.values),
                np.log10(halos.ypar.values),
                statistic="median",
                bins=bin_edges,
            )
            n, bin_edges, binnumber = stats.binned_statistic(
                np.log10(halos.m500.values),
                np.log10(halos.ypar.values),
                statistic="count",
                bins=bin_edges,
            )
            per10, bin_edges, binnumber = stats.binned_statistic(
                np.log10(halos.m500.values),
                np.log10(halos.ypar.values),
                statistic=lambda y: np.percentile(y, 10),
                bins=bin_edges,
            )
            per90, bin_edges, binnumber = stats.binned_statistic(
                np.log10(halos.m500.values),
                np.log10(halos.ypar.values),
                statistic=lambda y: np.percentile(y, 90),
                bins=bin_edges,
            )
            mask = np.where(n >= 1)
            ...
            inside = df_all.loc[
                (df_all.m500 >= 5.25e13) & (df_all.m500 <= 2e14)
            ]
            ...
            for j in range(len(centers)):
                ...
                df_i = inside.copy()
                df_i = df_i.loc[
                    (df_i.distance >= r_bins[j])
                    & (df_i.distance <= r_bins[j + 1])
                ]
                df_i["volume"] = (
                    (4.0 / 3)
                    * np.pi
                    * (
                        (
                            (df_i.r500 * aexp * u.Mpc.to(u.cm) * r_bins[j + 1])
                            ** 3.0
                        )
                        - (
                            (df_i.r500 * aexp * u.Mpc.to(u.cm) * r_bins[j])
                            ** 3.0
                        )
                    )
                )
                ...
                df_i["x_i"] = df_i.ne_nH / (
                    (df_i.ne_nH + df_i.ni_nH) * df_i.mu * mp
                )
            ...
            ax3.plot(
                centers,
                df_rho.median().values * (centers ** 2.0),
                lw=2,
                label=leg,
            )
            ...
            ax6.plot(
                centers,
                df_P.median().values * (centers ** 2.0),
                lw=2,
                label=leg,
            )
    ...
    ax1.set_xlabel(
        "$\mathrm{log}_{10} (M_{500, \mathrm{hse}} \,\, [\mathrm{M}_\odot])$"
    )
    ax1.set_ylabel(
        "$\mathrm{log}_{10} (L_{0.5-2.0 \mathrm{keV, hse}} \,\, [\mathrm{erg/s}])$"
    )
    ax1.set_xlim(13, 15)
    ...
    ax2.errorbar(
        planck_tSZ.x,
        planck_tSZ.y,
        yerr=[planck_tSZ.y - planck_tSZ.ymin, planck_tSZ.ymax - planck_tSZ.y],
        marker="o",
        ls="None",
        lw=1,
        capsize=2,
    ...
    ax2.set_ylabel(
        "$\mathrm{log}_{10} (\mathrm{E}(z)^{-2/3} Y_\mathrm{SZ, hse}(<5r_{500}) D_A^2 \,\, [\mathrm{Mpc}^2])$"
    )
    ax2.set_xlim(13, 15)
    ...
    y_rho = (
        10 ** sun_ne.iloc[:-2].log_ne * mu * mp / ne_n / rho_crit * x_rho ** 2
    )
    y_rho1 = (
        10 ** sun_ne.iloc[:-2].log_err_m
        * mu
        * mp
        / ne_n
        / rho_crit
    ...
    ax3.errorbar(
        x_rho[::3],
        y_rho[::3],
        yerr=[y_rho[::3] - y_rho1[::3], y_rho2[::3] - y_rho[::3]],
        marker="o",
        ls="None",
        lw=1,
        capsize=2,
    ...
    ax3.set_ylabel(
        "$\\rho/\\rho_\mathrm{crit} \,\, (r/r_{500, \mathrm{hse}})^2$"
    )
    ...
    ax5.errorbar(
        ...
        sun_S.S_S500_50,
        yerr=[
            sun_S.S_S500_50 - sun_S.S_S500_10,
            sun_S.S_S500_90 - sun_S.S_S500_50,
        ],
        marker="o",
        ls="None",
        lw=1,
    ...
    ax5.errorbar(
        ...
        ras_S.S_S500_50,
        yerr=[
            ras_S.S_S500_50 - ras_S.S_S500_10,
            ras_S.S_S500_90 - ras_S.S_S500_50,
        ],
        marker="s",
        ls="None",
        lw=1,
    ...
    x_P = 10 ** sun_P.log_r
    y_P = 10 ** sun_P.log_P / (fb / 0.175) * (0.73 / h) ** (8.0 / 3.0)

    err_P_m = (
        y_P - 10 ** sun_P.log_err_m / (fb / 0.175) * (0.73 / h) ** (8.0 / 3.0)
    ) * x_P ** 2
    err_P_p = (
        10 ** sun_P.log_err_p / (fb / 0.175) * (0.73 / h) ** (8.0 / 3.0) - y_P
    ) * x_P ** 2

    ax6.errorbar(
        x_P[1::3],
        y_P[1::3] * x_P[1::3] ** 2,
        yerr=[err_P_m[1::3], err_P_p[1::3]],
        marker="o",
        ls="None",
        lw=1,
        capsize=2,
    ...
    ax6.fill_between(
        bahamas_P.x, bahamas_P.ymin, bahamas_P.ymax, facecolor="C4", alpha=0.3
    )
    ...
    ax6.set_ylabel(
        "$P / P_{500, \mathrm{hse}} \,\, (r/r_{500, \mathrm{hse}})^2$"
    )
